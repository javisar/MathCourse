...
my $tmpdir = "C:\\temp\\pdfcrop_$random\\" ; 
mkdir( $tmpdir ) ; 
...
## run ghostscript
## open( BASH , "C:\\WINDOWS\\system32\\cmd.exe") || die "could not open BASH" ; 
## print BASH "gs.bat -dSAFER -dNOPAUSE -dBATCH -q -r72 -sDEVICE=bbox -f $infile 2> $gsfile" ; 
## close BASH ; 
system 1, "gs.bat -dSAFER -dNOPAUSE -dBATCH -q -r72 -sDEVICE=bbox -f $infile 2> $gsfile" ; 
...
##close BASH ; 
system 1, "D:\\TOOLS\\Pdfedit\\pdfedit.exe -console -run $qsfile $infile" ;
